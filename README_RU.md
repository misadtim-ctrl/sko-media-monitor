...
Городские паблики собирает **этот Mac, а не облако**: Instagram отвечает `429` на любой адрес дата-центра, и никакие паузы это не обходят.

Читается лента подписок — один запрос возвращает свежие посты всех пабликов сразу, поэтому проверка идёт **каждые 10 минут**, а не раз в час. Список подписок аккаунта и есть список того, что мониторится.

Порядок включения:

1. один раз [instagram-login.command](instagram-login.command) — сохранить вход;
2. [instagram-schedule.command](instagram-schedule.command) — включить расписание.

Выключить: `./instagram-schedule.command off`. Пока Mac спит, сбор не идёт; при молчании дольше трёх часов Apps Script присылает предупреждение в Telegram.

Второй бесплатный путь — официальный Meta API для Business/Creator-профилей — поддержан и включается заполнением `META_ACCESS_TOKEN` и `META_IG_USER_ID`. Он работает и из облака, но личные паблики через него не видны.
